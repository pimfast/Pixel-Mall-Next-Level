...
var _title = selectedmallpart.storename+" Lv"+string(selectedmallpart.level+1);
var _desc = selectedmallpart.leveldesc[selectedmallpart.level+1];
var _price = selectedmallpart.levelprice[selectedmallpart.level+1];
...
if (!variable_instance_exists(selectedmallpart.id,"levelpic")) {
	...
	_s = asset_get_index("spr_store_"+string(selectedmallpart.shopname)+"_lvl"+string(selectedmallpart.level+1));
	_sw = sprite_get_width(_s)*image_xscale;
	_sh = sprite_get_height(_s)*image_yscale;
	draw_sprite_stretched(_s,0,x-(_sw/2),y-(_sh/2)-(115*image_yscale),_sw,_sh);
} else {
	_s = selectedmallpart.levelpic[selectedmallpart.level+1];
	_sw = sprite_get_width(_s)*image_xscale;
	_sh = sprite_get_height(_s)*image_yscale;
	draw_sprite_stretched(_s,0,x-(_sw/2),y-(_sh/2)-(115*image_yscale),_sw,_sh);
}

if (!selectedmallpart.levelcurrency[selectedmallpart.level+1]) {
	_s = spr_icon_money;
} else {
	_s = spr_icon_point;
}
